...
def collect_data():
    ...
    try:
        ...
        # Yorumların yüklenmesi için scroll
                # Yorumların yüklenmesi için scroll
        body = driver.find_element(By.TAG_NAME, "body")

        last_height = driver.execute_script("return document.body.scrollHeight")
        same_height_count = 0  # üst üste kaç kez yükseklik değişmedi

        for m in range(1000):
            # Aşağı kaydır
            body.send_keys(Keys.PAGE_DOWN)
            time.sleep(0.7)  # biraz daha yavaş

            # Her 15 scroll'da bir, 5 kez yukarı çık
            if (m + 1) % 15 == 0:
                for _ in range(5):
            ...
            # Bilgi amaçlı log
            if (m + 1) % 50 == 0:
                print(f"Scroll sayısı: {m+1}")

            # Q'ya basılırsa manuel durdur
            if keyboard.is_pressed("q"):
                print("Q'ya basıldı, scroll durduruldu.")
                break

            # Yeni içerik gelip gelmediğini kontrol et
            ...
            # Birkaç tur üst üste yükseklik değişmiyorsa, sayfanın sonuna geldik
            if same_height_count >= 3:
                print("Yeni içerik yüklenmiyor, scroll döngüsü bitiriliyor.")
                break
